...
class FeaturesView(grok.View):
                    ...
                    if obj.values():
                        ...
                        for content in contents:
                            if content.portal_type == 'vindula.liberiuncontents.content.featuresection':
                                DC = {}
                                DC['image_content'] = '/++resource++vindula.liberiuncontents/imagens/img_box_secundario.jpg'
    ...
    def getContent(self):
        ...
        query = '/'.join(self.context.getPhysicalPath())
        results = pc( path={'query': query},
                     portal_type='vindula.liberiuncontents.content.featuresection',
                     review_state='published',
                     sort_on= 'getObjPositionInParent', )
        ...
        if results:
            for result in results:
                ...
                if obj.values():
                    topics = obj.values()
                    for topic in topics:
                        if topic.portal_type == 'vindula.liberiuncontents.content.featuretopic':
                            DT = {}
